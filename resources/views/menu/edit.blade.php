    ...
    <form action="{{ route('menu.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="productForm">
        ...
        <div class="mb-5">
            ...
            <div class="relative w-full h-32 border-2 border-dashed border-gray-200 rounded-2xl bg-white flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-50 transition-colors group overflow-hidden">
                <input type="file" name="image" id="inputImage" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10 form-input" accept="image/*">
                
                <div id="imagePlaceholder" class="flex flex-col items-center pointer-events-none {{ $product->image_url ? 'hidden' : '' }}">
                    ...
                    <p class="text-xs font-bold text-gray-700">Ganti Gambar</p>
                ...
                <img id="imagePreview" 
                     ...
                     class="absolute inset-0 w-full h-full object-cover {{ $product->image_url ? '' : 'hidden' }}" />
            </div>
        </div>
        ...
        <div class="mb-5">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <label class="cursor-pointer">
                    <input type="radio" name="kategori" value="{{ $cat }}" class="peer hidden form-input" 
                        {{ old('kategori', $product->kategori) == $cat ? 'checked' : '' }} required>
        ...
        <div class="mb-5">
            ...
            <div class="flex flex-wrap gap-2 mb-2">
                ...
                <label class="cursor-pointer flex-1">
                    <input type="radio" name="persen_keuntungan" value="{{ $percent }}" class="peer hidden percent-radio form-input" 
                        {{ old('persen_keuntungan', $product->persen_keuntungan) == $percent ? 'checked' : '' }} required>
        ...
        <div class="mb-8">
            ...
            <input type="text" id="hargaJualDisplay" readonly
                value="Rp {{ number_format($product->harga_jual, 0, ',', '.') }}"
        ...
        <div class="fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 p-5 z-50 max-w-[480px] mx-auto left-0 right-0">
            <button type="submit" id="submitBtn"
                class="w-full bg-[#37967D] text-white font-bold py-4 rounded-2xl shadow-lg shadow-[#37967D]/20 active:scale-95 transition-transform">
                Simpan Perubahan
            </button>
        </div>
    </form>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // PERBAIKAN DI SINI: Bungkus dengan kutip agar VS Code tidak error
        // Ambil nilai awal dari database untuk kalkulasi
        let currentPercent = parseFloat("{{ $product->persen_keuntungan ?? 0 }}");

        // 1. Logic Cek Kelengkapan Form
        function checkFormCompletion() {
            let allFilled = true;
            
            formInputs.forEach(input => {
                // Skip validasi gambar di halaman edit (karena gambar lama sudah ada)
                if (input.type === 'file') return;

                if (input.type === 'radio') {
                    const group = document.getElementsByName(input.name);
                    let groupChecked = false;
                    group.forEach(radio => { if (radio.checked) groupChecked = true; });
        ...
        // Jalankan cek awal agar tombol menyala jika data sudah ada
        checkFormCompletion();
    });
</script>
